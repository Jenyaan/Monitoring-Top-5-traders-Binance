...
def get_cookie(url, json_data, get_url):
    cookies = {
    ...
    headers = {
        ...
        'referer': f'https://www.binance.com/en/futures-activity/leaderboard/{url}',
    ...
    response = requests.post(
        f'https://www.binance.com/bapi/futures/{get_url}',
        cookies=cookies,
        headers=headers,
        json=json_data,
    ).json()
...
def rank_profile():
    ...
    list_r = get_cookie('futures', json_data, 'v3/public/future/leaderboard/getLeaderboardRank')
...
def profile_info(id_user,name_user):
    ...
    list = get_cookie(f'user/um?encryptedUid={id_user}', json_data ,'v2/public/future/leaderboard/getOtherPerformance')

    list = list.get('performanceRetList')
